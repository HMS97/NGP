...
resized_video = [cv2.resize(frame, (256, 256)) for frame in frame_list]
...
for i in range(0, len(resized_video), 4):
    # extract 4 frames
...
def multi_frame_Video_caption(time, modified_video):

    summary_text = f'The vision summary for frame {time} to {time+4} is: '
    time = time // 4
...
def combine_speech_video(time,modified_video,transformed_audio_result):
    visual_text =multi_frame_Video_caption(time = time, modified_video = modified_video)
    speech_text = find_near_speech_text(transformed_audio_result, T = time, time_range= 20, speaker_time = None)
    
    all_text =   f' Frame {time} to {time+4}: visual_text: ' + visual_text + ' speech_text: '+ speech_text + f" Frame {time} to {time+4} text end. "
    visual_text =   f' Frame {time} to {time+4}: visual_text: ' + visual_text + f" Frame {time} to {time+4} text end. "
    speech_text =   f' Frame {time} to {time+4}:  speech_text: '  + speech_text + f" Frame {time} to {time+4} text end. "
...
for index,time in enumerate(tqdm(range(0, len(resized_video), 4))):
    all_text, visual_text, speech_text = combine_speech_video(time = time, modified_video = modified_video, transformed_audio_result = transformed_audio_result)
...
# write_text(summary_text,'summary_all')
...
split_speech_file('data/speech_dir', all_speech_text,( numbers//1500 + 1))
